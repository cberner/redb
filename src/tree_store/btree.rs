        ...
        match node_mem[0] {
            ...
            BRANCH => {
                ...
                if let Some(new_aux_pn) = relocation_map.get(&old_aux_pn) {
                    ...
                    self.freed_pages.lock().unwrap().push(old_aux_pn);
                }
            ...
            _ => {
                // Aux page - content already copied above
            }
        }
        ...
        Ok(match node_mem[0] {
            ...
            BRANCH => {
                ...
                for i in 0..num_children {
                    ...
                    let child_cs = Checksum::from_le_bytes(
                        aux_page.memory()[offset..(offset + std::mem::size_of::<Checksum>())]
                            .try_into()
                            .unwrap(),
                    );
                    if !self.verify_checksum_helper(children[i], child_cs)? {
                        return Ok(false);
                    }
                }
                true
