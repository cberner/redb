...
impl Inner {
    ...
    fn revalidate_cache(&mut self, state: &SharedState, mem: &TransactionalMemory) {
        if self.cache_may_be_stale(state) {
            ...
            // Not simply the last commit: a read transaction this process already holds goes on
            // reading from its own, older snapshot, and every page it puts in the cache after this
            // comes from there. Raising the floor past it would assert something about the cache
            // that stops being true a moment later
            self.cache_floor = state.last_committed.min(self.registry.pinned());
        }
...
impl ProcessCoordinator {
    ...
    pub(crate) fn oldest_pinned_globally(
        ...
        let result = inner.registry.oldest_pinned();
        inner.registry.unlock();
    ...
    fn refresh_for_write(&self) -> Result<TransactionId> {
        ...
        // The file rather than the registry decides how far behind this process is. A writer that
        // made a commit durable and died before announcing it leaves the registry behind the file,
        // and a process whose own state matched that stale value would read it as "nothing has
        // changed", reuse the transaction id, and allocate over the pages that commit reaches
        let last_committed = self.mem.reload_transaction_slots()?;
