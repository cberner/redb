...
fn retain_collapses_to_single_entry() {
    ...
    {
        let mut table = write_txn.open_table(definition).unwrap();
        // Big enough to span several leaves and at least one internal branch
        // level.
        for i in 0u64..10_000 {
            table.insert(i, &[0u8; 200]).unwrap();
        }
        // Keep just one key, dropping everything else. Forces every leaf and
        // most branches to collapse, with exactly one survivor.
        table.retain(|k, _| k == 7777).unwrap();
        assert_eq!(table.len().unwrap(), 1);
        // Verify the survivor is the one we kept and that no stale entries
        // are reachable via iteration.
        let mut iter = table.iter().unwrap();
