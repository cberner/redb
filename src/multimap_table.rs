...
use crate::tree_store::{
    ...
    TransactionalMemory, WriteMemory, multimap_btree_stats,
};
...
pub struct MultimapValue<'a, V: Key + 'static> {
    ...
    // Present only when this iterator is backed by a write transaction and owns
    // pages that must be freed on drop. In read-only and standalone (inline)
    // iteration modes it's None, since the iterator doesn't allocate or free.
    write_mem: Option<WriteMemory>,
        ...
        let existed = if get_result.is_some() {
            ...
            match collection_type {
                Inline => {
                    ...
                    if required_inline_bytes < self.write_mem.mem().get_page_size() / 2 {
                        ...
                        let mut page = self.write_mem.allocate(leaf_data.len(), &mut allocated)?;
                        drop(allocated);
    ...
    pub fn remove<'k, 'v>(
        ...
        let existed = match v.collection_type() {
            ...
            SubtreeV2 => {
                ...
                {
                    let page = self.write_mem.mem().get_page(new_root, PageHint::None)?;
                    match page.memory()[0] {
                        LEAF => {
                            let accessor = LeafAccessor::new(
                                page.memory(),
                            ...
                            if len < self.write_mem.mem().get_page_size() / 2 {
                                let inline_data =
...
impl<K: Key + 'static, V: Key + 'static> ReadableMultimapTable<K, V> for MultimapTable<'_, K, V> {
    fn get<'a>(&self, key: impl Borrow<K::SelfType<'a>>) -> Result<MultimapValue<'_, V>> {
        ...
        let iter = if let Some(collection) = self.tree.get(key.borrow())? {
            MultimapValue::from_collection(collection, guard, self.write_mem.mem().clone())?
        } else {
            MultimapValue::new_subtree(
                BtreeRangeIter::new::<RangeFull, &V::SelfType<'_>>(
                    &(..),
