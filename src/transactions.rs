...
impl WriteTransaction {
    ...
    fn commit_inner(&mut self) -> Result<(), CommitError> {
        ...
        // Bypass DATA_FREED_TABLE for this transaction's frees when no live reader can observe
        // the about-to-be-replaced root: hand them straight to `commit_epilogue`, which returns
        // them to the allocator after `mem.commit`. Under quick-repair the saved snapshot
        // includes them via `try_save_allocator_state(..., pending_free_pages)`. Otherwise
        // (live reader, or non-durable commit) fall back to the freed tree so older readers
        // can still see the previous root's pages.
        ...
        // Drain any pages deferred by a previous epilogue (a reader raced in between that
        // commit's bypass decision and its mem.commit). They get stored in DATA_FREED_TABLE
        // below under their original txn ids, where a future `process_freed_pages` reclaims
        // them once the reader is gone.
    ...
    pub(crate) fn durable_commit(
        ...
        deferred_data_freed: BTreeMap<TransactionId, Vec<PageNumber>>,
    ) -> Result {
        ...
        // Persist deferred frees (from prior epilogues that hit a racing reader) to disk so a
        // crash doesn't lose them. Their original txn ids are still pinned by the reader, so
        // the freed-tree mutation here is safe (older entries) but `process_freed_pages` above
        // didn't touch them.
            ...
            system_tree.create_table_and_flush_table_root(
                ...
                |system_tree_ref, tree: &mut AllocatorStateTreeMut| {
                    ...
                    loop {
                        ...
                        // state matches the post-commit live state. Bypass-path data-tree
                        // frees ride into the saved snapshot via `try_save_allocator_state`'s
                        // `pending_free_pages` argument; the live allocator gets them in
                        // `commit_epilogue`, after `mem.commit`.
        ...
        self.commit_epilogue(data_freed_post_commit)?;

        Ok(())
    }

    // Releases `data_freed_post_commit` (the bypass set from `commit_inner`) to the live
    // allocator so the next commit can reuse those pages. Re-checks `oldest_live_read_transaction`
    // because a reader may have registered while fsync was running; if so, stash the pages via
    // `defer_data_freed` (the next durable commit drains the side list back into
    // `DATA_FREED_TABLE`).
    //
    // Crash safety: pages freed here are referenced by the on-disk secondary slot's data tree.
    // If the next write reuses one and crashes before its own `mem.commit`, recovery from
    // primary still succeeds; only secondary-fallback is at risk -- the same trade-off the
    // `system_freed_pages` drain just above (in `durable_commit`) already accepts.
    //
    // Deliberately does not mutate the freed trees: doing so post-commit would displace
    // freed-tree internal nodes belonging to the just-committed primary slot's system tree,
    // and a subsequent write reallocating them could overwrite their on-disk content before
    // its own `mem.commit`, corrupting the primary on a crash.
    fn commit_epilogue(&mut self, data_freed_post_commit: Vec<PageNumber>) -> Result {
