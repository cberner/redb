...
That check runs twice: once under the lock, which is authoritative, and once before it, which is what
keeps a `create()` from leaving a lock file in a directory it is about to refuse. The early one is
sound because a marker arrives by rename and so is either absent or complete: a file under that name
which is not a marker is not a state redb can produce, however many processes are working on the
directory at once. It only ever turns a call away -- accepting still waits for the lock.
...
Writing the marker last is what makes a directory without one ambiguous, so `create()` accepts an
unmarked directory only where it really could be an interrupted create of its own. `write.lock` is
the *first* thing `create()` makes, and nothing ever removes one, so every other name it writes
implies a lock file beside it. An unmarked directory holding one of those names *without*
`write.lock` was filled in by something else, and is refused.

Refusing it matters because of what `create()` would otherwise go on to do with those files. A
`data.redb` would be adopted and marked, converting somebody's plain `Database` into one of these --
the opposite of what the marker is for. A file under a temporary name would simply be deleted,
since redb unlinks those before writing them. Neither belongs in a directory this call was
pointed at by mistake.

Reading the lock file's absence that way is only sound if its own directory entry is durable before
anything else is written under these names, so `create()` fsyncs the directory once it has the lock.
Nothing orders one directory entry against another on its own, and fsyncing a file can carry the
entry that names it to disk as a side effect, so without that a crash could keep a `data.redb` this
call had just finished while losing the `write.lock` beside it -- and the next `create()` would read
