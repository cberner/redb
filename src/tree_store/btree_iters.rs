...
pub(crate) struct BtreeDrain<K: Key + 'static, V: Value + 'static> {
...
impl<K: Key, V: Value> BtreeDrain<K, V> {
    pub(crate) fn new(
        inner: BtreeRangeIter<K, V>,
        free_on_drop: Vec<PageNumber>,
        master_free_list: Arc<Mutex<Vec<PageNumber>>>,
...
impl<K: Key, V: Value> Iterator for BtreeDrain<K, V> {
...
impl<K: Key, V: Value> DoubleEndedIterator for BtreeDrain<K, V> {
    fn next_back(&mut self) -> Option<Self::Item> {
        self.inner.next_back()
    }
}

impl<K: Key, V: Value> Drop for BtreeDrain<K, V> {
    fn drop(&mut self) {
