...
* Add `Key::separator()`, which returns a short byte string that separates two keys, as a
  ...
  implementations unchanged; `&[u8]`, `&str`, and `String` keys now store minimal prefixes, and
  `Option`, tuple, and array keys shorten the element that the two keys first differ in, when
  that element is variable width.
